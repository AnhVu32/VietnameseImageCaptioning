    ...
    def get_batch_samples(self, dataset_split, img_idx_batch_list):
        ...
        for i in range(len(img_idx_batch_list)):
            ...
            if dataset_split == CocoDatasetKarpathy.TestSet_ID:
                ...
                caption_id = 99  # Gán giá trị mặc định để debug
                try:
                    captions_list = self.coco_dataset.karpathy_val_list[img_idx]['captions']
                    print(f"Image index {img_idx} has {len(captions_list)} captions.")  # In số lượng captions
                    caption_id = random.randint(a=0, b=len(captions_list)-1)
                    caption = captions_list[caption_id]
                except IndexError as e:
                    print(f"Error: Index out of range for image index {img_idx}")
                    print(f"Attempted caption_id: {caption_id}")
                    print(f"Captions list: {captions_list}")  # In danh sách captions
                    print(f"Total captions count: {len(captions_list)}")
                    raise e
            else:

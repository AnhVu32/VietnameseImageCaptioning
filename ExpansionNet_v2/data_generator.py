...
def generate_data(path_args):
    ...
    coco_dataset = CocoDatasetKarpathy(images_path=path_args.images_path + 'train-images/',
                                       coco_annotations_path=path_args.images_path + "train_data.json",
    ...
    with torch.no_grad():
        ...
        for i in range(coco_dataset.val_num_images):
            ...
            if path_args.dtype == 'fp16':
                ...
                hdf5_file.create_dataset(str(img_id) + '_features', data=np.array(output.cpu()))
            if (i+1) % 2500 == 0 or (i+1) == coco_dataset.val_num_images:
                print("Val " + str(i+1) + " / " + str(coco_dataset.val_num_images) + " completed")
